...
**数据流**：前端 `api/` 层（axios）→ Express 路由 → `store.js` 读取/写入 JSON 文件 → 返回数据 → Pinia 更新状态 → 视图渲染。音频与封面作为静态资源由后端托管，前端通过 Vite 代理直接访问。
...
| 📊 性能优化 | 首屏骨架屏、路由懒加载、手动分包、内存缓存、性能监控（FCP/LCP/CLS） |
...
│   │   ├── utils/                #   工具函数（request/errorHandler/perf 等）
...
| `/mine` | 我的 | 用户中心、收藏、播放历史（Tabbar 一级页） | ✅ |
...
> 认证接口统一返回 `{ token, user }`；错误响应统一 `{ error: "描述" }`。
