import { createApp } from 'vue'
import pinia from '@/stores/index'
import App from './App.vue'
import router from './router'
import { setupErrorHandlers, settleChunkRecovery } from '@/utils/errorHandler'
import { initTelemetry } from '@/utils/telemetry'
import '@/utils/perf'

// Vant 模板组件样式由 VantResolver 按需引入（vite.config.js）；
// 这里集中补 JS 函数式组件（showToast/showNotify/showConfirmDialog）的样式入口，
// 页面里只用 import { xx } from 'vant'，无需逐个补样式
import 'vant/es/toast/style'
import 'vant/es/notify/style'
import 'vant/es/dialog/style'
import '@/styles/notify.css'
import '@/styles/global.css'

const app = createApp(App)

app.use(pinia)
app.use(router)

// 全局错误捕获：Vue 渲染/全局 error/unhandledrejection/路由 chunk 兜底
setupErrorHandlers(app, router)

// 计数采集：注册页面隐藏/关闭时的上报点（track 可被任何模块直接调用，无需等待初始化）
initTelemetry()

// 首屏路由就绪（含懒加载页面 chunk 预加载）后再挂载：
// 挂载前 index.html 内联骨架持续可见，挂载瞬间路由一次渲染到位，
// 避免"骨架标题 → 真实标题"或"骨架 → 空白 → 内容"的闪烁
router.isReady().then(() => {
  app.mount('#app')
  // 本次启动若由 chunk 自动恢复 reload 而来且成功到达目标路由 → 计 recover.chunk.success
  settleChunkRecovery(router)
})
